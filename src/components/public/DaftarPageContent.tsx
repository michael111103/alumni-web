...
import {
 ...
 Globe, Package, FileText, Camera, Plus, Trash2, Calendar
} from 'lucide-react'
...
const emptyUMKM = (): UMKMForm => ({
 namaUsaha: '', tahunBerdiri: '', kategoriUsahaId: '', deskripsiUsaha: '', skalaUsaha: '',
       ...
       for (const u of umkmForms) {
         ...
         const { data: umkm, error: umkmErr } = await supabase
           ...
           .insert({
             ...
             tahun_berdiri: u.tahunBerdiri ? parseInt(u.tahunBerdiri) : null,
   ...
   <div className="border rounded-2xl p-4 space-y-4 relative" style={{ borderColor: '#E0DDD8', background: '#FAFAFA' }}>
     ...
     <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
       ...
       <div>
         ...
         <Input value={u.tahunBerdiri} onChange={e => onUpdate(idx, 'tahunBerdiri', e.target.value)}
           placeholder="cth: 2020" type="number" min="1900" max={new Date().getFullYear()}
