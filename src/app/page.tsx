  ...
  return (
    <main className="min-h-screen" style={{ background: '#FAF8F4', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
        ...
        <div className="max-w-5xl mx-auto px-6 pt-8 pb-0">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 items-end">
            ...
            <div className="hidden md:grid grid-cols-2 gap-2 items-end pb-0">
              <div
                className="h-40 rounded-t-2xl relative flex items-center justify-center overflow-hidden"
              ...
              <div
                className="h-28 rounded-t-2xl mt-12 relative flex items-center justify-center overflow-hidden"
      ...
      <div
        ...
        style={{ background: '#C0272D', maxWidth: '100%' }}
      >
      ...
      <section style={{ background: '#FAF8F4' }} className="px-6 py-6">
        <div className="max-w-5xl mx-auto">
          <div className="flex items-baseline justify-between mb-4">
            ...
            <Link
              href="/umkm"
              className="text-xs border-b pb-0.5 transition hover:opacity-70"
            ...
            {[
              ...
              >
                ...
                <div className="absolute bottom-2 left-2.5">
                  <span
                    className="text-white/85 text-xs font-medium px-2 py-0.5 rounded-full"
          ...
          <div className="grid grid-cols-3 gap-3">
            {recentAlumni.length > 0
              ? recentAlumni.map((alumni, idx) => {
                  ...
                  if (alumni.jabatan) tags.push({ label: alumni.jabatan, style: { background: '#EFEFED', color: '#6B6B6B' } })
                  if (hasUMKM) tags.push({ label: 'UMKM', style: { background: '#EFEFED', color: '#6B6B6B' } })

                  return (
                    <Link key={alumni.id} href={`/alumni/${alumni.id}`}>
                      <div
                        className="border rounded-xl p-3.5 cursor-pointer transition-all hover:border-red-500 hover:shadow-sm h-full"
                        style={{ borderColor: '#E0DDD8' }}
                      >
                        <div
                          className="w-10 h-10 rounded-full flex items-center justify-center text-xs font-bold mb-2.5"
                        ...
                        {tags.length > 0 && (
                          <div className="flex flex-wrap gap-1 mb-1.5">
                            {tags.map(t => (
                              <span key={t.label} className="text-xs px-1.5 py-0.5 rounded-full" style={t.style}>
                                {t.label}
                              </span>
                            ))}
                          </div>
    ...
    return (
      ...
      >
        ...
        <p className="text-sm" style={{ color: '#6B6B6B' }}>
          Belum ada UMKM featured minggu ini
        </p>
      </div>
    )
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
      ...
      <Link href={`/alumni/${main.alumni_id || ''}`}>
        <div
          className="rounded-2xl p-5 relative overflow-hidden h-full min-h-[220px] flex flex-col justify-between cursor-pointer hover:opacity-95 transition"
        ...
        >
          ...
          <div>
            <div
              className="flex items-center gap-1.5 text-xs mb-2.5 uppercase tracking-widest"
            ...
            <h3
              className="font-bold text-xl text-white leading-tight mb-1.5"
              style={{ fontFamily: "'Playfair Display', serif" }}
            >
              {main.nama_usaha}
            </h3>
            <p className="text-xs leading-relaxed mb-3" style={{ color: 'rgba(255,255,255,0.55)' }}>
              {main.deskripsi_usaha?.slice(0, 90)}
              {main.deskripsi_usaha?.length > 90 ? '...' : ''}
            </p>
            {main.kategori_usaha && (
        ...
        {subs.length > 0
          ? subs.map((item, i) => (
              <Link key={item.id} href={`/alumni/${item.alumni_id || ''}`}>
                <div
                  className="bg-white rounded-xl p-4 border cursor-pointer hover:border-red-400 transition h-full"
                  style={{
                    borderColor: '#E0DDD8',
                    borderLeft: `3px solid ${i === 0 ? '#C0272D' : '#2A2A2A'}`,
                  }}
                >
                  <div
                  ...
                  <p className="text-xs leading-relaxed mb-3" style={{ color: '#6B6B6B' }}>
                    {item.deskripsi_usaha?.slice(0, 65)}
                    {item.deskripsi_usaha?.length > 65 ? '...' : ''}
                  </p>
                  <div className="flex items-center justify-between">
